---
title: "Redraw a Graphic Chart for PWD Analysis Results"
...
reanalysis of results.  The final results are available in the Excel File in
the "Final Fact Sheet Scores" worksheet.
...
## Adjust Lake names
We want to convert names to title case. A method using `gsub()` does the trick.
The basic idea is from a stackoverflow question here:
...
```{r graphic_no_totals, fig.height = 7, fig.width = 4}
plt +
...
We would also like to be able to add an annotation marker to totals calculated
with partial data.
...
```{r graphic_totals, fig.height = 7, fig.width = 4}
plt2 +
...
```{r graphic_totals_horizontal, fig.height =3, fig.width = 7}
plt3 +
  scale_fill_manual(values = altcolors_2)
